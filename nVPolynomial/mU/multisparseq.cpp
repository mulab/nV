...
void MultiAddQ(sparse_q & rr,const sparse_q & f,const sparse_q & g,order_q order)
{
	size_t sf,sg,i,j;
	sf=f.size();sg=g.size();
	sparse_q r;
	...
	while(1)
	{
		...
		if(flag==0)
		{
			mpq_t temp;
			mpq_init(temp);
			mpq_add(temp,f[i].coe,g[j].coe);
			if(mpq_sgn(temp)!=0){//not zero
				r.resize(r.size()+1);
				mpq_set(r.right().coe,temp);
				r.right().exponents=f[i].exponents;
			}
			i++;j++;
			continue;
...
void MultiNegQ(sparse_q & rr,const sparse_q & f){
	sparse_q r;
	r.resize(f.size());
	for(size_t i=0;i<f.size();i++){
		mpq_neg(r[i].coe,f[i].coe);
