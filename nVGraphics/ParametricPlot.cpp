...
namespace nV {
namespace Graphics {
ParametricPlot::ParametricPlot(Kernel &k, const Tuple &cmd) : k(k) {
    ...
    if(expr.isTuple($.List)) {
        if(expr.tuple()[expr.tuple().size - 1].isTuple($.List)) {
    ...
    for(uint i = 0; i < curnum; i++) {
        var funs = expression.tuple()[i + 1];
        F1P *fx = new F1P(k, funs.tuple()[1], tparam);
        F1P *fy = new F1P(k, funs.tuple()[2], tparam);
        if (!colorFunctionSet)
...
var ParametricPlot::exportGraphics() {
    var shapelist;
    if (!colorFunctionSet) {
...
void ParametricPlot::setRule(var title, var rule) {
