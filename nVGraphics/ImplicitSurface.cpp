...
// 		return;
    if (std::abs(i) > m_bounds || std::abs(j) > m_bounds || std::abs(k) > m_bounds)
        return;
...
ImplicitSurface::CORNER* ImplicitSurface::setcorner (int i, int j, int k) {
    ...
    l->j = j;
    l->k = k;
    l->value = c->value = getFunctionValueSafely(f, c->x, c->y, c->z);
...
ImplicitSurface::TEST ImplicitSurface::find (int sign, double x, double y, double z) {
    ...
    for (i = 0; i < 10000; i++) {
        ...
        test.value = getFunctionValueSafely(f, test.p.x, test.p.y, test.p.z);
...
double getFunctionValueSafely(F3P *f_s, double x, double y, double z) {
    double ans = f_s->getSingleData(x, y, z);
    ...
    if(ans == -std::numeric_limits<double>::infinity()) {
        ans = std::numeric_limits<double>::min();
        return ans;
    }
