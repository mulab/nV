...
API wstring path();
API bool shell(wcs);
API wstring to_wstring(const char *src, size_t len);
API string to_string(const wchar *src, size_t len);
...
#define __FUNCTIONW__ _STR2WSTR(__PRETTY_FUNCTION__)
...
API	wstring current_path();
API	string dump_path();
}
