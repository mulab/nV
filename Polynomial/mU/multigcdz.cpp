...
void MultiGcdZ_sparse2(sparse_z & r, const sparse_z & skel, const sparse_z & l, const sparse_z & f, const sparse_z & g, uint Tk, uint k, uint D)
{
	if(k==0)
	{
		...
		mpz_t c;mpz_init(c);
		mpz_set(c,l[0].coe);
		for(signed long i=0;i<((signed long) rr.size())-1;++i)
		{
			mpz_mul(rr[i],rr[i],c);
...
void MultiGcdZ_Sparse1(sparse_z & r, const sparse_z & l, const sparse_z & f, const sparse_z & g, uint k, uint D)
{
	...
	mpz_set_ui(high,10+d*d*d);
	mpz_neg(low,high);
MultiGcdZ_Sparse1_restart:
	...
	if(!MultiDivisibleZ(f,r)||!MultiDivisibleZ(g,r))
		goto MultiGcdZ_Sparse1_restart;
...
void MultiGcdZ_Sparse(sparse_z & r, const sparse_z & f, const sparse_z & g, uint n)
{
	...
	for(uint i=0;i<fc.size();++i)
		MultiMulZ(fc[i],fc[i],l);
	for(uint i=0;i<gc.size();++i)
		MultiMulZ(gc[i],gc[i],l);
...
void MultiLcmZ(sparse_z & r, const sparse_z & f,const sparse_z & g)
{
	sparse_z gcd, t;
	MultiGcdZ(gcd, f, g);
	MultiDivExactZ(t, f, gcd);
	MultiMulZ(r, t, g);
	gcd.resize(0); t.resize(0);
}
