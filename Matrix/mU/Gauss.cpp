...
var qMinors(var M, size_t k)
{
	size_t m=Size(M);
	size_t n=Size(At(M,0));
	scoped_array<size_t> c(new size_t[k]);
	scoped_array<size_t> d(new size_t[k]);
	...
	do
	{
		...
		do
		{	
			Entry(R,i,j)=qMinor(M,c.get(),d.get(),k);
			++j;
		}while(next_comb(d.get(),n,k));
		++i;
	}while(next_comb(c.get(),m,k));
