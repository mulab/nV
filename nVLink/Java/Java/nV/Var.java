...
public final class Var {
	...
	public Var(MExpr val) {
		MExpr head = val.head();
		if (head == MExpr.SYMBOLexpr) {
			this.type = SYMBOL;
			//this.val = Symbol.get(val.toString());
			this.val = Symbol.system(val.toString());
		} else if (head == MExpr.INTEGERexpr) {
			this.type = INTEGER;
			this.val = new BigInteger(val.toString());
		} else if (head == MExpr.REALexpr) {
			this.type = REAL;
			this.val = new BigDecimal(val.toString());
		} else if (head == MExpr.STRINGexpr) {
			this.type = STRING;
			this.val = val.toString();//toJavaString?
		} else {
			this.type = TUPLE;
			int length = val.length();
			Var[] t = new Var[length + 1];
			for (int i = 0; i < length + 1; ++i)
				t[i] = new Var(val.part(i));
			this.val = t;
		}
	}
