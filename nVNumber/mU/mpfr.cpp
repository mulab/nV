...
namespace {
inline mpf_ptr mpf_new(long prec = 0)
{
	mpf_ptr r = new __mpf_struct;
	if (prec)
		mpf_init2(r, prec);
	else
		mpf_init2(r, mpf_get_default_prec());
	return r;
}
inline mpf_ptr mpf_new(mpf_ptr x)
{
	mpf_ptr r = mpf_new(mpf_get_prec(x));
	mpf_set(r,x);
	return r;
}
...
{
	mpf_ptr r = mpf_new(mpfr_get_prec(x));
	mpfr_get_f(r,x,GMP_RNDD);
	return r;
}
inline mpfr_ptr mpfr_new(long prec = 0)
{
...
var name(Var x)\
{\
	mpfr_ptr rr = mpfr_new(mpf_get_prec(CFlt(x)));\
